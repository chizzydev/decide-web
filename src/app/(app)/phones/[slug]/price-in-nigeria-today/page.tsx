import type { Metadata } from 'next'
import { notFound } from 'next/navigation'
import { PhonePricingSeoPage } from '@/components/market/PhonePricingSeoPage'
import {
  buildPhonePricingIntentMetadata,
  getPhonePricingSeoBundle,
} from '@/lib/phonePricingSeo'
import { parseVariantIdFromSearchParam } from '@/lib/variantHref'

interface PriceTodayPageProps {
  params: Promise<{ slug: string }>
  searchParams?: Promise<{ variant_id?: string | string[] }>
}

export async function generateMetadata({
  params,
  searchParams,
}: PriceTodayPageProps): Promise<Metadata> {
  try {
    const { slug } = await params
    const resolvedSearchParams = (await searchParams) ?? {}
    const variantId = parseVariantIdFromSearchParam(resolvedSearchParams.variant_id)
    const bundle = await getPhonePricingSeoBundle(slug, { variantId })
    return buildPhonePricingIntentMetadata('price-in-nigeria-today', bundle)
  } catch {
    return {
      title: 'Phone price in Nigeria today - Decide',
      description: 'See the latest tracked phone price in Nigeria today and compare trusted stores.',
    }
  }
}

export default async function PriceTodayPage({
  params,
  searchParams,
}: PriceTodayPageProps) {
  try {
    const { slug } = await params
    const resolvedSearchParams = (await searchParams) ?? {}
    const variantId = parseVariantIdFromSearchParam(resolvedSearchParams.variant_id)
    const bundle = await getPhonePricingSeoBundle(slug, { variantId })
    return <PhonePricingSeoPage intent="price-in-nigeria-today" bundle={bundle} />
  } catch {
    notFound()
  }
}
